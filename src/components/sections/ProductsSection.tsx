      ...
      <div className="relative z-[2]">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-2xl">
            <Reveal>
              <div className="flex items-center gap-3">
                ...
                <span className="label-tech-on-dark">Our products</span>
              </div>
            </Reveal>
            <KineticHeading
              as="h2"
            ...
            />
            <Reveal delay={0.1}>
              <p className="mt-4 text-base leading-8 text-white/70 sm:mt-6 sm:text-lg">
          ...
          <Reveal delay={0.05}>
            <Link
              href="/products"
              className="inline-flex shrink-0 items-center gap-2 text-sm font-medium text-malachite transition hover:text-malachite/90"
            >
              View all products
            ...
            return (
              <Reveal key={product.id} delay={0.08 + i * 0.06}>
                <Link
                  ...
                  className={cn(
                    greenBandCardClassName(
                      "group flex h-full flex-col overflow-hidden hover:shadow-glow-green",
                    ),
                  )}
                >
                  <div
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      {product.tags.map((tag) => (
                        <span key={tag} className={greenBandChipClassName}>
                          {tag}
                        </span>
                      ))}
                    </div>
                    <Magnetic strength={0.25} className="mt-6 w-fit">
                      <span className="label-tech-on-dark inline-flex items-center gap-2 text-malachite transition group-hover:gap-3">
                        Explore product
